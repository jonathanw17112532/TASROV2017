...
//const static uint8_t i2c_bus_addresses[9] = {0, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B};
const static uint8_t i2c_bus_addresses[9] = {0, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B};
static uint8_t i2c_motor_fault[8] = {0, 0, 0, 0, 0, 0, 0, 0};
const static uint8_t i2c_register_address = 0x00;

volatile static uint8_t servo1_angle = 90;
volatile static uint8_t servo2_angle = 90;
...
void motorWrite(uint8_t channel, uint8_t direction, uint8_t power) {
  if (direction) bitSet(power, 0);
  else bitClear(power, 0);
